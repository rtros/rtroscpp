#ifndef RTAI_THREAD_THREAD_COMMON_HPP
#define RTAI_THREAD_THREAD_COMMON_HPP
...
//            {}
    ...
    //public:
        ...
        {
            return detail::thread_data_ptr(boost::detail::heap_new<detail::thread_data<F> >(f));
        }
        template<typename F>
        static inline detail::thread_data_ptr make_thread_info(BOOST_THREAD_RV_REF(F) f)
        {
    ...
    public:
        ...
        {
          boost::chrono::milliseconds rel_time2= boost::chrono::ceil<boost::chrono::milliseconds>(rel_time);
        ...
        bool try_join_for(const boost::chrono::duration<Rep, Period>& rel_time)
        {
          return try_join_until(boost::chrono::steady_clock::now() + rel_time);
        }
#endif
        ...
        bool try_join_until(const boost::chrono::time_point<boost::chrono::system_clock, Duration>& t)
        {
          using namespace boost::chrono;
    ...
    public:
        bool timed_join(const boost::system_time& abs_time);
        //{
        //  return do_try_join_until(get_milliseconds_until(wait_until));
        //}

#ifdef BOOST_THREAD_USES_CHRONO
        bool try_join_until(const boost::chrono::time_point<boost::chrono::system_clock, boost::chrono::nanoseconds>& tp)
        {
          boost::chrono::milliseconds rel_time= boost::chrono::ceil<boost::chrono::milliseconds>(tp-boost::chrono::system_clock::now());
    ...
    public:
      ...
      public:
        ...
        {
            return timed_join(boost::get_system_time()+rel_time);
        }
#endif
        void detach();
...
#define BOOST_THREAD_DEFINES_THREAD_NATIVE_HANDLE
        ...
        static inline void sleep(const boost::system_time& xt)
        {
    ...
    namespace this_thread
    {
        ...
        {
            sleep(boost::system_time(abs_time));
        }
#endif
    }
    ...
    private:
        ...
        hash_value(const thread::id &v)
        {
          ...
          return boost::hash_value(v.thread_data);
#else
          return boost::hash_value(v.thread_data.get());
#endif
        }
  ...
  std::basic_ostream<charT, traits>&
    ...
    void thread::join() {
        if (this_thread::get_id() == get_id())
          boost::throw_exception(boost::thread_resource_error(static_cast<int>(boost::system::errc::resource_deadlock_would_occur), "boost thread: trying joining itself"));
...
#else
        void BOOST_THREAD_DECL make_ready_at_thread_exit(boost::shared_ptr<shared_state_base> as);
#endif
    }
    ...
    namespace this_thread
    {
        ...
        {
            detail::thread_exit_function_base* const thread_exit_func=boost::detail::heap_new<detail::thread_exit_function<F> >(f);
